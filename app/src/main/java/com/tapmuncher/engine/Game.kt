...
class Game(private val store: SettingsStore, private val host: GameHost) {
        ...
        for ((i, g) in ghosts.withIndex()) {
            ...
            when (i) {
                0 -> { g.inHouse = false; g.x = DOOR_X.toFloat(); g.y = (DOOR_Y - 1).toFloat(); g.dir = 2 }
                    ...
                    // Hold the pack back on the opening levels. All four were
                    // loose within 8.5s at level 1, which gives a new player no
                    // room to read the maze before being converged on.
                    val grace = (4 - level).coerceAtLeast(0)
                    g.exitAt = time + 1.5f + grace * 1.6f + (i - 1) * (3.5f + grace * 1.1f)
                }
            }
        }
    }
    ...
    /** Levels 1-3 run the pack slower so the maze can be learned before it
     *  starts hunting properly. Full speed from level 4 on. */
    private fun earlyLevelEase(): Float =
        when (level) { 1 -> 0.86f; 2 -> 0.91f; 3 -> 0.96f; else -> 1f }
